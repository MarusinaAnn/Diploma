...
def get_current_user(request):
    ...
    return Response({
        ...
        "email": user.email,
        "is_admin": user.is_superuser  # или user.is_staff, если ты используешь его
    })
...
class FileViewSet(viewsets.ModelViewSet):
    ...
    def rename(self, request, pk=None):
        ...
        if serializer.is_valid():
            file.display_name = serializer.validated_data['new_name']
    ...
    def access_download(self, request, access_url=None):
        ...
        if os.path.exists(absolute_path):
            ...
            filename_utf8 = iri_to_uri(file.display_name)
            response['Content-Disposition'] = f"attachment; filename*=UTF-8''{filename_utf8}"
            logger.info(f"Файл {file.display_name} был скачан по публичной ссылке: {access_url}")
            return response
